...
// -> ANTI FAKE CLIENT
extern BOOL		ser_AntiFakeClientBug_bActive = TRUE;					 //if playing a default map it's not be necessary
...
extern BOOL		ser_AntiF9_bActive			= TRUE;
...
public:
	...
	CClientList()
	{
	  ...
	  for(i=0; i<CT_TOBEKICKEDPERSONS; i++)
	  {
		tbkKickMe[i].bUsed = FALSE;
      }
	  for(i=0; i<CT_F9CANDIDATES; i++)
	  {
...
extern BOOL		ser_AntiSwimBug_bKick = 1;
...
extern INDEX ser_bUseCustomCRCList = 1;
...
void CGame::InitInternal( void)
{
  ...
  _pShell->DeclareSymbol("persistent user INDEX	ka_bEnabled;",							&ka_bEnabled);	
  _pShell->DeclareSymbol("persistent user FLOAT	ka_tmMaxOffset;",						&ka_tmMaxOffset);
  _pShell->DeclareSymbol("persistent user INDEX	ka_bKillingSpree;",						&ka_bKillingSpree);
  _pShell->DeclareSymbol("persistent user INDEX	ka_bMultiKill;",						&ka_bMultiKill);
  _pShell->DeclareSymbol("persistent user INDEX	ka_bTrimNamesLeft;",					&ka_bTrimNamesLeft);	
  _pShell->DeclareSymbol("persistent user INDEX	ka_bUndecorateNames;", 					&ka_bUndecorateNames);
  _pShell->DeclareSymbol("persistent user INDEX	ka_bOneLinePerMsg;",					&ka_bOneLinePerMsg);

  _pShell->DeclareSymbol("persistent user INDEX	ka_ctmkKillsForAward[7];",				&ka_ctmkKillsForAward);
  _pShell->DeclareSymbol("persistent user INDEX	ka_ctksKillsForAward[6];",				&ka_ctksKillsForAward);

  _pShell->DeclareSymbol("persistent user CTString ka_strmkDoubleKill;",				&ka_strmkDoubleKill);
  _pShell->DeclareSymbol("persistent user CTString ka_strmkMultiKill;",				    &ka_strmkMultiKill);
  _pShell->DeclareSymbol("persistent user CTString ka_strmkMegaKill;",				    &ka_strmkMegaKill);
  _pShell->DeclareSymbol("persistent user CTString ka_strmkUltraKill;",				    &ka_strmkUltraKill);
  _pShell->DeclareSymbol("persistent user CTString ka_strmkMonsterKill;",				&ka_strmkMonsterKill);
  _pShell->DeclareSymbol("persistent user CTString ka_strmkLudicrousKill;",				&ka_strmkLudicrousKill);
  _pShell->DeclareSymbol("persistent user CTString ka_strmkHolyShit;",				    &ka_strmkHolyShit);

  _pShell->DeclareSymbol("persistent user CTString ka_strksKillingSpree;",				&ka_strksKillingSpree);
  _pShell->DeclareSymbol("persistent user CTString ka_strksRampage;",					&ka_strksRampage);	
  _pShell->DeclareSymbol("persistent user CTString ka_strksDominating;",				&ka_strksDominating);
  _pShell->DeclareSymbol("persistent user CTString ka_strksUnstoppable;",				&ka_strksUnstoppable);
  _pShell->DeclareSymbol("persistent user CTString ka_strksGodlike;",					&ka_strksGodlike);
  _pShell->DeclareSymbol("persistent user CTString ka_strksWickedSick;",				&ka_strksWickedSick);
  //[Tobias B] -> 1.08 Feature: Kill awards P3
  // -> ANTI FAKE CLIENT 
  _pShell->DeclareSymbol("persistent user INDEX		ser_AntiFakeClientBug_bActive;",			&ser_AntiFakeClientBug_bActive);
  _pShell->DeclareSymbol("persistent user FLOAT		ser_AntiFakeClientBug_tmStartPacketCheckOffset;",	&ser_AntiFakeClientBug_tmStartPacketCheckOffset);
  _pShell->DeclareSymbol("persistent user FLOAT		ser_AntiFakeClientBug_tmPacketCheckDuration;",		&ser_AntiFakeClientBug_tmPacketCheckDuration);

  _pShell->DeclareSymbol("persistent user INDEX		ser_AntiF9_bActive;",						&ser_AntiF9_bActive);
  _pShell->DeclareSymbol("persistent user INDEX		ser_AntiF9_ctMaxF9s;",						&ser_AntiF9_ctMaxF9s);			
  _pShell->DeclareSymbol("persistent user FLOAT		ser_AntiF9_tmRememberF9s;",					&ser_AntiF9_tmRememberF9s);
  _pShell->DeclareSymbol("persistent user FLOAT		ser_AntiF9_tmWaitTime;",					&ser_AntiF9_tmWaitTime);
  // <- ANTI FAKE CLIENT 


  _pShell->DeclareSymbol("user void AddModel(CTString, CTString, FLOAT, FLOAT, FLOAT, FLOAT, FLOAT, FLOAT, INDEX, FLOAT, INDEX);",        &AngeloModel);
  _pShell->DeclareSymbol("user CTString RemoveSpecialCodes(CTString);", &RemoveSpecialCodes);
  _pShell->DeclareSymbol("user CTString RemoveAll(CTString, CTString);", &strRemoveAll);
  ...
  _pShell->DeclareSymbol("user INDEX rf_iMode;",		  &rf_iMode);
  _pShell->DeclareSymbol("user INDEX rf_iWeaponSet;",	  &rf_iWeaponSet);
  _pShell->DeclareSymbol("user INDEX rf_bRemovePowerups;",&rf_bRemovePowerups);
  _pShell->DeclareSymbol("user INDEX rf_bRemoveSupers;",  &rf_bRemoveSupers);
  _pShell->DeclareSymbol("user INDEX rf_bRemoveCannon;",  &rf_bRemoveCannon);
  _pShell->DeclareSymbol("user INDEX rf_bRemoveGrenades;",&rf_bRemoveGrenades);
  _pShell->DeclareSymbol("user INDEX rf_bChangeLights;",  &rf_bChangeLights);
  _pShell->DeclareSymbol("user FLOAT rf_fDeathmatchHP;",  &rf_fDeathmatchHP);
  _pShell->DeclareSymbol("user INDEX rf_bRandomLights;",  &rf_bRandomLights);
  _pShell->DeclareSymbol("user INDEX rf_iLights;",		  &rf_iLights);
  _pShell->DeclareSymbol("user INDEX rf_iLights2;",		  &rf_iLights2);
  _pShell->DeclareSymbol("user INDEX rf_iLights3;",		  &rf_iLights3);
  _pShell->DeclareSymbol("user INDEX rf_iLights4;",		  &rf_iLights4);
  _pShell->DeclareSymbol("user INDEX rf_iLights5;",		  &rf_iLights5);
  //_pShell->DeclareSymbol("user CTString rf_strSpawnMsg;", &rf_strSpawnMsg);
  //_pShell->DeclareSymbol("user CTString rf_cmdOnJoin",    &rf_fnmOnJoin);
  _pShell->DeclareSymbol("user CTString rf_cmdOnKill;",   &rf_cmdOnKill);
  //_pShell->DeclareSymbol("user CTString rf_cmdOnLeave;",  &rf_fnmOnLeave);
  _pShell->DeclareSymbol("user CTString rf_cmdUser1;",   &rf_cmdUser1);
  _pShell->DeclareSymbol("user CTString rf_cmdUser2;",   &rf_cmdUser2);
  _pShell->DeclareSymbol("user CTString rf_cmdGun;",     &rf_cmdGun);
  _pShell->DeclareSymbol("user INDEX rf_cmdSuicide;",	 &rf_cmdSuicide);
  _pShell->DeclareSymbol("user INDEX rf_bEmptyWorld;", &rf_bEmptyWorld);
  _pShell->DeclareSymbol("user INDEX rf_bTiltedWorld;", &rf_bTiltedWorld);
  _pShell->DeclareSymbol("user INDEX rf_bOpacityEnable;", &rf_bOpacityEnable);
  _pShell->DeclareSymbol("user FLOAT rf_fOpacityAmount;", &rf_fOpacityAmount);
  _pShell->DeclareSymbol("user INDEX rf_bTestPlayer;", &rf_bTestPlayer);

	_pShell->DeclareSymbol("user INDEX rf_bModifySpawners;", &rf_bModifySpawners);
	_pShell->DeclareSymbol("user FLOAT rf_fEnemySpawnerCountMultiplier;", &rf_fEnemySpawnerCountMultiplier);
	_pShell->DeclareSymbol("user FLOAT rf_fEnemySpawnerDelayMultiplier;", &rf_fEnemySpawnerDelayMultiplier);
	_pShell->DeclareSymbol("user FLOAT rf_fEnemySpawnerCircleMultiplier;", &rf_fEnemySpawnerCircleMultiplier);

  _pShell->DeclareSymbol("user INDEX ser_bUseCustomCRCList;", &ser_bUseCustomCRCList);
		...
		if(ser_bUseCustomCRCList)
		{
			...
			if(ba!=NULL)
			{
				...
				while(sa_strOriginalFilesAndNames.Count()>1)
				{
					...
					CTString &strpub = CTString((const char*)pub);
	...
	while(bFound){
      ...
      if(rf_bEmptyWorld) {
        ...
        continue;
			...
			{
				if(((CWeaponItem*)pen)->m_EwitType==WIT_CANNON && rf_bRemoveCannon==1){
					pen->Destroy();
					bFound=TRUE;
				}else if(((CWeaponItem*)pen)->m_EwitType==WIT_GRENADELAUNCHER && rf_bRemoveGrenades==1){
					pen->Destroy();
					bFound=TRUE;
				}
			}
